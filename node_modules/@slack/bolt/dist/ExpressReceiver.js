...
class ExpressReceiver extends events_1.EventEmitter {
    constructor({ signingSecret = '', logger = new logger_1.ConsoleLogger(), endpoints = { events: '/slack/events' }, agent = undefined, clientTls = undefined, }) {
        super();
...
function verifySignatureAndParseBody(logger, signingSecret) {
    return async (req, _res, next) => {
        try {
            ...
            const { 'x-slack-signature': signature, 'x-slack-request-timestamp': requestTimestamp, 'content-type': contentType, } = req.headers;
            await verifyRequestSignature(signingSecret, stringBody, signature, requestTimestamp);
            // *** Parsing body ***
...
async function verifyRequestSignature(signingSecret, body, signature, requestTimestamp) {
    if (signature === undefined || requestTimestamp === undefined) {
        throw errors_1.errorWithCode('Slack request signing verification failed. Some headers are missing.', errors_1.ErrorCode.ExpressReceiverAuthenticityError);
    }
    const ts = Number(requestTimestamp);
    if (isNaN(ts)) {
        throw errors_1.errorWithCode('Slack request signing verification failed. Timestamp is invalid.', errors_1.ErrorCode.ExpressReceiverAuthenticityError);
    }
    // Divide current date to match Slack ts format
    // Subtract 5 minutes from current time
    const fiveMinutesAgo = Math.floor(Date.now() / 1000) - (60 * 5);
    if (ts < fiveMinutesAgo) {
        throw errors_1.errorWithCode('Slack request signing verification failed. Timestamp is too old.', errors_1.ErrorCode.ExpressReceiverAuthenticityError);
    }
    const hmac = crypto_1.default.createHmac('sha256', signingSecret);
    const [version, hash] = signature.split('=');
    hmac.update(`${version}:${ts}:${body}`);
    if (!tsscmp_1.default(hash, hmac.digest('hex'))) {
        throw errors_1.errorWithCode('Slack request signing verification failed. Signature mismatch.', errors_1.ErrorCode.ExpressReceiverAuthenticityError);
    }
}
